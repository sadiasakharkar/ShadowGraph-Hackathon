        ...
        for n in nodes:
            await session.run(
                ...
                SET x.label = $label, x.node_type = $node_type, x.suspicious = $suspicious,
                    x.verified = $verified, x.metadata = $metadata
                """,
                id=n.id,
                user_id=user_id,
                label=n.label,
...
async def read_graph(user_id: str) -> tuple[list[dict], list[dict]]:
    ...
    async with db.neo4j_driver.session() as session:
        nodes_result = await session.run(
            "MATCH (n:Entity {user_id: $uid}) RETURN n.id as id, n.label as label, n.node_type as node_type, n.suspicious as suspicious, n.verified as verified, n.metadata as metadata",
            uid=user_id,
        )
