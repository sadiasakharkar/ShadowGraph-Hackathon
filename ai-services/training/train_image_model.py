...
from app.ml.features import VisionEmbedder, image_pair_features
...
def load_rows() -> tuple[np.ndarray, np.ndarray]:
    ...
    with DATA.open("r", encoding="utf-8") as f:
        ...
        for row in reader:
            x.append(image_pair_features(embedder, row["image_a"], row["image_b"]))
...
def train() -> dict:
    ...
    joblib.dump(model, MODELS / "image_similarity_model.joblib")
    report = {
